...
5. **Análisis de opiniones UX/UI externas y plan de mejoras** — pendiente de ejecutar (2026-07-21). El usuario pasó 6 "opiniones de expertos" externas sobre el sitio; 2 de ellas evaluaban directamente otro sitio (no aplican) y 2 afirmaciones puntuales (falta de formulario de contacto, problemas de contraste) se refutaron con evidencia — ver análisis completo y plan priorizado en `docs/superpowers/specs/2026-07-21-mejoras-ux-analisis.md`. Resumen de lo confirmado como real: falta Open Graph (`og:image`, etc. — cero en `Layout.astro`), `ProblemaGrid` duplicado idéntico en `/` y `/servicios`, prueba social no verificable, "IA" usada como muletilla sin explicar qué hace, nav mezcla rutas reales y anclas, sin dominio propio, sin foto de Juancito/equipo, Hero saturado en mobile, scroll-reveal puede parpadear en blanco con scroll rápido (pendiente de confirmar visualmente). Nada de esto se tocó todavía — **empezar por acá la próxima sesión**.
...
8. Google Analytics (GA4) — pendiente, el usuario va a pasar el código más adelante (mismo patrón que Meta Pixel: nuevo componente env-configurable en `src/components/`).
