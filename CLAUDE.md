...
8. ~~Google Tag Manager~~ ✅ instalado (2026-07-21). `src/components/GoogleTagManager.astro` (mismo patrón que `MetaPixel.astro`: env-configurable vía `PUBLIC_GTM_ID`, solo dispara en build de producción), con dos `placement` (`"head"` — script principal, insertado lo más arriba posible en `<head>` — y `"body"` — `<noscript><iframe>`, insertado justo después de `<body>`, antes de `NavBar`). ID real (`GTM-WFWTVSKT`, provisto por el usuario) ya está en `.env` local. **Pendiente antes de que funcione en producción:** configurar `PUBLIC_GTM_ID=GTM-WFWTVSKT` en el dashboard de Netlify del proyecto `juancitoads` (Environment variables) y forzar redeploy — mismo paso que se hizo con `PUBLIC_FB_PIXEL_ID` en el punto 2.
